...
void Depixelizing::showSimilarityGraph()
{
    ...
    cv::Mat graph(scale * m_height, scale * m_width, CV_8UC3, cv::Scalar(0,0,0));
    for (int i = 0; i < m_height; ++i)
    {
        for (int j = 0; j < m_width; ++j)
        {
            ...
            {
                cv::line(graph, cv::Point( (scale + 1) / 2 + j * scale, (scale + 1) / 2 + i * scale), cv::Point( (scale + 1) / 2 + j * scale, (scale + 1) / 2 + (i + 1) * scale), cv::Scalar(255,255,255));
            }
            if (m_connections[i * m_width + j] & NEIGHBOUR_BOTTOM_RIGHT)
            {
                cv::line(graph, cv::Point( (scale + 1) / 2 + j * scale, (scale + 1) / 2 + i * scale), cv::Point((scale + 1) / 2 + (j + 1) * scale, (scale + 1) / 2 + (i + 1) * scale), cv::Scalar(255,255,255));
            }
            ...
            {
                cv::line(graph, cv::Point( (scale + 1) / 2 + j * scale, (scale + 1) / 2 + i * scale), cv::Point((scale + 1) / 2 + (j + 1) * scale, (scale + 1) / 2 + i * scale), cv::Scalar(255,255,255));
            }
...
void Depixelizing::showCellImage()
{
    ...
    const std::string outname = m_outname;
